...
func TestHeaderCreation(t *testing.T) {
	...
	header := constructNodeHeader(1, 0, 4, 1, 0, NODE_HEADER_SIZE_V1)
...
func sendTwoDataPackets() ([]byte, []byte) {
	...
	nh1 := constructNodeHeader(1, 2, uint16(2), uint16(2), uint16(len(data)), NODE_HEADER_SIZE_V1)
	packet := &nodePacket{
		nh1,
		[]byte(data),
	}
	...
	nh2 := constructNodeHeader(1, 2, uint16(2), uint16(2), uint16(len(data2)), NODE_HEADER_SIZE_V1)
	packet2 := &nodePacket{
		nh2,
		[]byte(data2),
	}
...
func sendOnePacket() []byte {
	...
	nh2 := constructNodeHeader(1, 1, uint16(2), uint16(2), uint16(len(data2)), NODE_HEADER_SIZE_V1)
	packet2 := &nodePacket{
		nh2,
		[]byte(data2),
	}
